...
async def give_premium_cmd_handler(client, message):
    if len(message.command) == 4:
        time_zone = datetime.datetime.now(pytz.timezone("Bangladesh/Noakhali"))
        ...
        if seconds > 0:
            ...
            expiry_str_in_ist = expiry.astimezone(pytz.timezone("Bangladesh/Noakhali")).strftime("%d-%m-%Y\n⏱️ ᴇxᴘɪʀʏ ᴛɪᴍᴇ : %I:%M:%S %p")
            expiry_str_ist = expiry.astimezone(pytz.timezone("Bangladesh/Noakhali")).strftime("%d-%m-%Y 𝘈𝘵 : %I:%M:%S %p")         
...
async def myplan(client, message):
    ...
    if data and data.get("expiry_time"):
        expiry = data.get("expiry_time") 
        expiry_ist = expiry.astimezone(pytz.timezone("Bangladesh/Noakhali"))
        expiry_str_in_ist = expiry.astimezone(pytz.timezone("Bangladesh/Noakhali")).strftime("%d-%m-%Y  ⏰: %I:%M:%S %p")            
        current_time = datetime.datetime.now(pytz.timezone("Bangladesh/Noakhali"))
